...
import type { IfoodTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<IfoodTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as IfoodTokenSet;
    } catch (error) {
  ...
  async write(tokens: IfoodTokenSet): Promise<void> {
