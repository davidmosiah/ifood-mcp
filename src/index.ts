...
import { registerIfoodTools } from "./tools/ifood-tools.js";
...
async function runHttp(): Promise<void> {
  ...
  const port = Number(process.env.IFOOD_MCP_PORT ?? 3000);
...
if (cliResult !== undefined) {
  ...
  const transport = process.env.IFOOD_MCP_TRANSPORT ?? (args.has("--http") ? "http" : "stdio");
  if (transport === "http") {
    await runHttp();
  } else {
    await runStdio();
